...
@vsc.randobj
class rgu_apb_space_sequence(uvm_sequence):
    ...
    async def body(self):
        ...
        if self.addr > 0xfff and self.direction == apb_direction.WRITE:
            address = int((bin(self.addr)[2:-2] + '00')[-12:], 2)

            if address == RGU_REG_GLB_ADDR: # prohibition of transactions with access to the RGU_GLB register to prevent unexpected resets
