...
@vsc.randobj
class rgu_reset_space_sequence(uvm_sequence):
    """Creating a reset signal with duration conditions"""
    def __init__(self, name):
        super().__init__(name)
        self.clk_cycles_after = vsc.rand_bit_t(32)
        self.clk_cycles_before = vsc.rand_bit_t(32)
        self.clk_cycles_duration = vsc.rand_bit_t(32)

    @vsc.constraint
    def clk_cycles_constr(self):
        self.clk_cycles_after < RESET_COMFORT_DURATION_LIMIT
        self.clk_cycles_before < RESET_COMFORT_DURATION_LIMIT
        self.clk_cycles_duration < RESET_COMFORT_DURATION_LIMIT and self.clk_cycles_duration > RESET_DURATION_LIMIT_MIN

    async def body(self):
        item = reset_driver_item(reset_action.RST_PULSE)
        item.clk_cycles_before = self.clk_cycles_before
